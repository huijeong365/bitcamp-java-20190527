...
//    tail = head; // head의 값을 tail에 저장하라.
  ...
  public boolean add(T value) {
    ...
    if (head == null)
      head = temp;
    if (tail != null)
      tail.next = temp; // 현재 tail에 뭔가의 node가 있을때 사용 가능
    tail = temp;
    size++;
...
//    size++;
  ...
  public T get(int index) {
    ...
    Node<T> node = head; // 임시 node 를 만들어 head의 주소 저장
  ...
  public T remove(int index) {
    ...
    deletedNode.value = null; // 삭제될 노드가 다른 객체를 참조하지 않도록 초기화 시킨다.
    deletedNode.next = null; // 이런 식으로 개발자가 메모리 관리게 기여할 수 있다.
  ...
  public Object[] toArray() {
    ...
    // LinkedList의 head에서 tail 까지 반복하면서 배열에 value를 복사한다.
    // 방법1:
    /*
    Node node = head;
