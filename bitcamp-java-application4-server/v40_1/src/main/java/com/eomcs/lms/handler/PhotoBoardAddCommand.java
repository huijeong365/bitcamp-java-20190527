...
public class PhotoBoardAddCommand implements Command {
  ...
  public void execute(BufferedReader in, PrintStream out) {
    ...
    try {
      ...
      // => 따라서 이 메서드가 끝날 때 까지 DAO는 같은 커넥션을 사용할것이다.
      //
      con = conFactory.getConnection();
      // DAO가 사용할 커넥션에 대해 AutoCommit을 false로 설정한다.
      // => DBMS 쪽 담당자(스레드)에게 지금 부터 수행하는 모든 데이터 변경(insert, update, delete) 작업은 
      //    임시 데이터 베이스에 보관 하라고 명령을 내린다.
      ...
      // => DBMS 쪽 담당자에게(스레드)에게 이제부터 모든 데이터 변경 작업은 즉시 실행할 것을 명령한다.
